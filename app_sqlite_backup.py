...
def admin_users():
    ...
    cur.execute('SELECT id, prenom, nom, email, active, is_admin, created_at FROM users ORDER BY created_at DESC')
    users = cur.fetchall()
...
def rdkq_participer():
    ...
    try:
        cur.execute('''INSERT INTO submissions (nom, email, sujet, message, rgpd, attachment, created_at)
                       VALUES (?, ?, ?, ?, ?, ?, ?)''', (nom, email, sujet, message, int(bool(rgpd)), saved_filename, now))
        db.commit()
...
DB_PATH = os.environ.get('UMAN_DB_PATH', os.path.join(os.path.dirname(__file__), 'data', 'uman.db'))
...
def handle_domain():
    ...
    if session.get('user_id'):
        db = get_db()
        cur = db.cursor()
        cur.execute('SELECT is_admin FROM users WHERE id = ? AND active = 1', (session['user_id'],))
        user = cur.fetchone()
...
def register():
    ...
    now = datetime.now(timezone.utc).isoformat()
    try:
        cur.execute('''INSERT OR REPLACE INTO users (prenom, nom, email, organisation, telephone, code_hash, code_expires, active, created_at)
               VALUES ((SELECT prenom FROM users WHERE email = ?), (SELECT nom FROM users WHERE email = ?), ?, ?, ?, ?, ?, 0, COALESCE((SELECT created_at FROM users WHERE email = ?), ?))''',
                    (email, email, email, organisation, telephone, code_hash, expires, email, now))
        db.commit()
    except Exception as e:
        ...
        row = cur.fetchone()
        if row:
            cur.execute('UPDATE users SET prenom=?, nom=?, organisation=?, telephone=?, code_hash=?, code_expires=? WHERE email=?',
                        (prenom, nom, organisation, telephone, code_hash, expires, email))
        else:
            cur.execute('INSERT INTO users (prenom, nom, email, organisation, telephone, code_hash, code_expires, active, created_at) VALUES (?,?,?,?,?,?,?,?,?)',
                        (prenom, nom, email, organisation, telephone, code_hash, expires, 0, now))
        db.commit()
...
def register_confirm():
    ...
    if row['active']:
        # Utilisateur déjà actif, mettre à jour la session
        session['user_id'] = row['id']
        session['user_email'] = row['email']
        return jsonify({'ok': True, 'message': 'already active'})

    if row['code_expires'] is None:
        return jsonify({'ok': False, 'error': 'no code set'}), 400
    # parse ISO stored string into aware datetime
    expires = datetime.fromisoformat(row['code_expires']) if isinstance(row['code_expires'], str) else row['code_expires']
    ...
    cur.execute('UPDATE users SET active=1, code_hash=NULL, code_expires=NULL WHERE email=?', (email,))
    db.commit()
